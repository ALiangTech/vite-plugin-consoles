...
import addArgumentToConsoleLog from "./ast";

const createScript = async () => {
  const filePath =
    path.resolve() +
    "/node_modules/vite-plugin-consoles/public/proxy-console.js";
...
const createSourcePosition = (args: string, options: ConsolePluginOptions) => {
  const {
    sourceStyle = "color:#646cff; background-color: #efdbff ;display:flex;padding: 8px",
  } = options;
  return `originConsole.log('%c console.log(${args.replace(
    /"'/g,
    '"'
  )}) 输出的源码位置', "${sourceStyle}");`;
};
    ...
    transform(src, id) {
      ...
      if (filter(id)) {
        ...
        [...matchs].forEach((item) => {
          ...
          //     replaceMatch = `${createSourcePosition(
          //       args,
          //       options
          //     )}${temp},['isPlugin',${tempArgs}]);`;
          //     src = src.replace(matchStr, replaceMatch);
          //   } else {
          //     src = src.replace(
          //       matchStr,
          //       matchStr.replace(/console/g, "originConsole")
          //     ); // 走原始打印
          //   }
          // }
        });
